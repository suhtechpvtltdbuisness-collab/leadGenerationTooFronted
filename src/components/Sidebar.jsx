    ...
    return (
        <div className="w-[240px] h-screen bg-white flex flex-col fixed left-0 top-0 overflow-hidden border-r border-gray-200">
            ...
            <nav className="flex-1 px-3 py-2 overflow-y-auto scrollbar-hide">
                <div className="mb-4">
                    <div className="text-[10px] uppercase tracking-wider text-gray-400 font-bold px-3 mb-2">Menu</div>
                    {menuItems.map((item) => (
                        <button
                            key={item.id}
                            ...
                            recentLeads.map((lead, idx) => (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        <span className="text-[9px] text-yellow-600 font-bold">⭐ {lead.rating}</span>
                                        <span className="text-[9px] text-gray-400">•</span>
                                        <span className="text-[9px] text-gray-400 truncate">{new Date(lead.createdAt).toLocaleDateString()}</span>
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="px-3 py-4 text-center">
                                <p className="text-[10px] text-gray-400 italic">No leads stored yet</p>
                            </div>
                        )}
                    </div>
                </div>
